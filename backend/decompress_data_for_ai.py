...
class DataDecompressor:
            ...
            if not result['readings'] or len(result['readings']) == 0:
                ...
                try:
                    ...
                    if isinstance(timestamp, datetime):
                        ...
                        try:
                            # Thử đầu tiên với định dạng ISO
                            dt = datetime.fromisoformat(timestamp.replace('Z', '+00:00'))
                        except ValueError:
                            try:
                                # Thử với định dạng không có timezone và microsecond
                                if 'T' in timestamp and not '.' in timestamp:
                                    dt = datetime.strptime(timestamp, '%Y-%m-%dT%H:%M:%S')
                                else:
                                    # Xử lý format khác nếu cần
                                    dt = datetime.strptime(timestamp, '%Y-%m-%d %H:%M:%S.%f')
                            except ValueError as e:
                                logger.warning(f"Không thể parse timestamp {timestamp}: {str(e)}")
                                dt = None
                    else:
    ...
    def convert_to_dataframe(self, decompressed_data: List[Dict[str, Any]]) -> pd.DataFrame:
        ...
        # Chuyển đổi timestamp sang datetime với format='mixed' để tự phát hiện định dạng
        if "timestamp" in df.columns:
            try:
                # Sử dụng format='mixed' để pandas tự phát hiện định dạng
                df["timestamp"] = pd.to_datetime(df["timestamp"], format='mixed')
            except Exception as e:
                logger.warning(f"Lỗi khi chuyển đổi timestamp: {str(e)}")
                logger.warning("Sẽ để nguyên dạng chuỗi cho timestamp")
