...
def generate_template_data(num_days: int = 7, device_id: str = "template_two", start_date: Optional[datetime] = None, season: Optional[str] = None) -> List[Dict[str, Any]]:
    """
...
def main():
    ...
    if args.start_date:
        try:
            ...
            # Đặt thời gian về 00:00:00 để bắt đầu từ đầu ngày
    ...
    # Hiển thị thông tin về sử dụng mẫu
    logger.info("Các mẫu tiêu thụ điện đa dạng đã được tạo bao gồm:")
    logger.info("1. Khác biệt theo ngày trong tuần (Thứ 2 -> Chủ nhật)")
    logger.info("2. Khác biệt theo thời gian trong ngày (Sáng sớm, Buổi sáng, Ban ngày, Buổi tối, Đêm muộn)")
    logger.info("3. Khác biệt theo mùa (Mùa Xuân, Hè, Thu, Đông)")
    logger.info("4. Khác biệt theo tuần trong tháng (Tuần đầu, giữa, cuối)")
    logger.info("Các yếu tố ngẫu nhiên và đột biến cũng được thêm vào để tạo dữ liệu chân thực hơn.")
